...
Thanks for the interest! buddy is a tiny project with a small surface area. Most contributions land cleanly if you read this first.

## Local setup

```bash
git clone https://github.com/AlexFloyd13/buddy ~/path/to/buddy
cd ~/path/to/buddy
...
- Brand-new user (no save file)
- Missing `transcript_path` field
- Malicious `/etc/passwd`-style transcript path
- Corrupted save (wrong-type fields)
- Empty `{}` save
- Narrow terminal (40 cols)
- Huge `buddy_col` (10 billion)
...
Use the templates in `.github/ISSUE_TEMPLATE/`. The "bug report" template asks for a render dump (`python3 ~/.claude/buddy/statusline.py < /dev/null`) which makes alignment / color bugs reproducible.
